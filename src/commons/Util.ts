...
export function getColorInArray(
  array: (string | Color)[],
  index: number,
  total: number,
) {
  const v = (index / total) * array.length;
  const p = v % 1;
  const f = Math.floor(v);
  const c = Math.ceil(v);
  const floorColor = array[f];
  const ceilColor = array[c];
  return Color.lerp(floorColor, ceilColor, p);
}
